...
summon minecraft:arrow ~ ~1.62 ~ {Tags:["PortalBulletTemp","PortalProjectileTemp","PortalBullet","PortalProjectile","NoPortal","OrangePortal"],damage:0,life:1195,Invulnerable:1,NoGravity:1,Silent:0,Color:16351261,Passengers:[{id:"minecraft:item",Age:5995,PickupDelay:32767,Invulnerable:1,Item:{id:"minecraft:leather_boots",Count:1,tag:{Damage:2,Unbreakable:1,Temp:{X:0,Y:0,Z:0},ShooterRotation:[0.0f,0.0f],display:{color:16351261}}},Tags:["PortalItemTemp","PortalProjectileTemp","PortalItem","PortalProjectile","NoPortal","OrangePortal"]}]}
execute if score @s PortalAltColor matches 0.. store result entity @e[tag=PortalBulletTemp,limit=1] Color int 1 run scoreboard players get @s PortalAltColor
execute if score @s PortalAltColor matches 0.. store result entity @e[tag=PortalBulletTemp,limit=1] Passengers[0].Item.tag.display.color int 1 run scoreboard players get @s PortalAltColor
...
execute at @e[tag=PortalBulletTemp] as @e[tag=PortalBullet,tag=OrangePortal,distance=0.1..] if score @s DerpcraftOwner = @e[tag=PortalBulletTemp,limit=1] DerpcraftOwner at @s run kill @e[tag=PortalProjectile,limit=2,sort=nearest]
